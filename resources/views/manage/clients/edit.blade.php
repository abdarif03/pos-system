...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-600">Edit data client: {{ $client->name }}</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form method="POST" action="{{ route('manage.clients.update', $client) }}">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="package_type" name="package_type" 
                        ...
                        <option value="basic" {{ old('package_type', $client->package_type) == 'basic' ? 'selected' : '' }}>Basic</option>
                        <option value="premium" {{ old('package_type', $client->package_type) == 'premium' ? 'selected' : '' }}>Premium</option>
                        <option value="enterprise" {{ old('package_type', $client->package_type) == 'enterprise' ? 'selected' : '' }}>Enterprise</option>
                    </select>
                    @error('package_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" id="registration_date" name="registration_date" 
                           value="{{ old('registration_date', $client->registration_date->format('Y-m-d')) }}" 
            ...
            <div class="mt-6">
                ...
                <textarea id="notes" name="notes" rows="4" 
                          ...
                          placeholder="Enter additional notes">{{ old('notes', $client->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
