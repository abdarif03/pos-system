                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                ...
                                <a href="{{ route('manage.payments.edit', $payment) }}" class="text-indigo-600 hover:text-indigo-900">
                                    <i class="fas fa-edit"></i>
                                </a>
                                @if($payment->status == 'pending')
                                <form method="POST" action="{{ route('manage.payments.approve', $payment) }}" class="inline">
                                    @csrf
                                    <button type="submit" class="text-green-600 hover:text-green-900" onclick="return confirm('Approve this payment?')">
                                        <i class="fas fa-check"></i>
                                    </button>
                                </form>
                                <form method="POST" action="{{ route('manage.payments.reject', $payment) }}" class="inline">
                                    @csrf
                                    <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Reject this payment?')">
